...
wget -c ftp://ftp.ncbi.nlm.nih.gov/pub/clinvar/xml/ClinVarFullRelease_00-latest.xml.gz -P output/
wget -c ftp://ftp.ncbi.nlm.nih.gov/pub/clinvar/tab_delimited/variant_summary.txt.gz -P output/
...
cat <(gunzip -c ./output/clinvar_table_normalized.single.b37.tsv.gz | head -1) <(gunzip -c ./output/clinvar_table_normalized.single.b37.tsv.gz | tail -n +2 | egrep -v "^[XYM]" | sort -k1,1n -k2,2n -k3,3 -k4,4 ) <(gunzip -c ./output/clinvar_table_normalized.single.b37.tsv.gz | tail -n +2 | egrep "^[XYM]" | sort -k1,1 -k2,2n -k3,3 -k4,4 )  | bgzip -c > ./output/clinvar_allele_trait_pairs.single.b37.tsv.gz                                    
